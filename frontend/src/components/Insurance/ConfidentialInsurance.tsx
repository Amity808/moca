...
import { generateJwt, getJwtPayload, EXAMPLE_JWKS_URL, validatePrivateKey } from "../../utils";
...
const CredentialIssuance = ({
  ...
  const [privateKeyValidation, setPrivateKeyValidation] = useState<{ isValid: boolean; format?: string; error?: string } | null>(null);

  // Configuration - these would typically come from environment variables or API
  const [config, setConfig] = useState({
    issuerDid: process.env.NEXT_PUBLIC_ISSUER_DID || "did:example:issuer123",
    credentialId:
      process.env.NEXT_PUBLIC_CREDENTIAL_ID || "c21hc0g0joevn0015479aK",
  });
  ...
  const [kid, setKid] = useState(
    process.env.NEXT_PUBLIC_KID || ""
  );

  const [jwtAlgorithm, setJwtAlgorithm] = useState<"ES256" | "RS256">(
    (process.env.NEXT_PUBLIC_JWT_ALGORITHM as "ES256" | "RS256") || "ES256"
  );
  ...
  console.log('Button disabled check:', {
    isLoading,
    isLoggedIn,
    hasGeneratedJwt: !!generatedJwt,
    hasIssuerDid: !!config.issuerDid,
    hasCredentialId: !!config.credentialId,
    hasCredentialFields: Object.keys(convertFieldsToCredentialSubject()).length > 0,
    credentialFieldsCount: Object.keys(convertFieldsToCredentialSubject()).length
  });
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <textarea
                  ...
                  className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-brand-500 resize-vertical ${privateKeyValidation?.isValid === false
                    ? 'border-red-300 bg-red-50'
                    : privateKeyValidation?.isValid === true
...
-----END PRIVATE KEY-----"
                ...
                {privateKeyValidation && (
                  <div className={`mt-2 text-xs ${privateKeyValidation.isValid ? 'text-green-600' : 'text-red-600'
                    }`}>
                    {privateKeyValidation.isValid ? (
                      <span>✅ Valid {privateKeyValidation.format} format</span>
                    ) : (
                      <div>
                        <div>❌ {privateKeyValidation.error}</div>
                        {privateKeyValidation.error?.includes('PKCS#1') && (
                          <div className="mt-1 text-xs text-gray-600">
                            <strong>Convert with OpenSSL:</strong><br />
                            <code className="bg-gray-100 px-1 rounded">openssl pkcs8 -topk8 -inform PEM -outform PEM -nocrypt -in rsa_key.pem -out pkcs8_key.pem</code>
                          </div>
                        )}
                        {privateKeyValidation.error?.includes('EC') && (
                          <div className="mt-1 text-xs text-gray-600">
                            <strong>Convert with OpenSSL:</strong><br />
                            <code className="bg-gray-100 px-1 rounded">openssl pkcs8 -topk8 -inform PEM -outform PEM -nocrypt -in ec_key.pem -out pkcs8_key.pem</code>
                          </div>
                        )}
                      </div>
                    )}
        ...
        {isDisabled && (
          <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
            ...
            <ul className="text-xs text-yellow-700 space-y-1">
              ...
              {Object.keys(convertFieldsToCredentialSubject()).length === 0 && <li>• Please add at least one credential field with a name and value</li>}
            </ul>
          </div>
        )}
